...
const endpoint = `${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/api/v1/companies/credits/:credit_id`;
...
describe("PATCH /api/v1/companies/credits/:credit_id", () => {
  ...
  describe("Authenticated user", () => {
    describe("User has a incorrect role", () => {
      test("Should return 403 status when the user is a consumer", async () => {
        ...
        const endpointWithCreditId = endpoint.replace(":credit_id", new mongoose.Types.ObjectId().toString());

        const response = await fetch(endpointWithCreditId, {
          method: "PATCH",
          headers: {
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after update a company credit status", async () => {
        ...
        const endpointWithCreditId = endpoint.replace(":credit_id", credit.documentsCreatedOnMongo[0]._id.toString());
      ...
      test("Should return 400 when the status is not valid", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
            excluded: false,
        ...
        expect(response.status).toBe(400);
        expect(body.name).toBe("ValidationError");
        expect(body.message).toBe(localize("error.generic.invalid", { field: "status" }));
        expect(body.action).toBe(localize("error.ValidationError.action"));
      });
    });
  });
