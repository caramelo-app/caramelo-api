...
describe("GET /api/v1/companies/users/:user_id", () => {
  describe("Anonymous user", () => {
    test("Should return 401 status when the user is not sending a token", async () => {
      const userId = new mongoose.Types.ObjectId().toString();

      const endpointWithUserId = endpoint.replace(":user_id", userId);
  ...
  describe("Authenticated user", () => {
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after get the company consumers", async () => {
        ...
        expect(body.created_at).toBe(user.documentsCreatedOnMongo[0].created_at.toISOString());
