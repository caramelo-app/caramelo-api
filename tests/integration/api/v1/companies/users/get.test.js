...
const endpoint = `${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/api/v1/companies/users`;
...
describe("GET /api/v1/companies/users", () => {
  ...
  describe("Authenticated user", () => {
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after get the company users", async () => {
        ...
        const user = await orchestrator.createDocumentOnMongo(3, userHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
        ...
        const endpointWithPagination = `${endpoint}?limit=10&skip=0`;
        ...
        expect(body[0].name).toBe(user.documentsCreatedOnMongo[0].name);
        expect(body[0].status).toBe(statusConsts.RESOURCE_STATUS.AVAILABLE);
        expect(body[1].name).toBe(user.documentsCreatedOnMongo[1].name);
        expect(body[1].status).toBe(statusConsts.RESOURCE_STATUS.AVAILABLE);
        expect(body[2].name).toBe(user.documentsCreatedOnMongo[2].name);
        expect(body[2].status).toBe(statusConsts.RESOURCE_STATUS.AVAILABLE);
      });
