...
const endpoint = `${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/api/v1/companies/cards`;
...
describe("GET /api/v1/companies/cards", () => {
  ...
  describe("Authenticated user", () => {
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after get the company cards", async () => {
        ...
        expect(body[0]._id).toBe(card.documentsCreatedOnMongo[0]._id.toString());
        expect(body[0].title).toBe(card.documentsCreatedOnMongo[0].title);
        expect(body[0].credits_needed).toBe(card.documentsCreatedOnMongo[0].credits_needed);
        expect(body[0].credit_expires_at.ref_number).toBe(card.documentsCreatedOnMongo[0].credit_expires_at.ref_number);
        expect(body[0].credit_expires_at.ref_type).toBe(card.documentsCreatedOnMongo[0].credit_expires_at.ref_type);
        expect(body[0].status).toBe(card.documentsCreatedOnMongo[0].status);
        expect(body[0].count.credits).toBe(2);
        expect(body[0].count.consumers).toBe(1);
        expect(body[1]._id).toBe(card.documentsCreatedOnMongo[1]._id.toString());
        expect(body[1].title).toBe(card.documentsCreatedOnMongo[1].title);
        expect(body[1].credits_needed).toBe(card.documentsCreatedOnMongo[1].credits_needed);
        expect(body[1].credit_expires_at.ref_number).toBe(card.documentsCreatedOnMongo[1].credit_expires_at.ref_number);
        expect(body[1].credit_expires_at.ref_type).toBe(card.documentsCreatedOnMongo[1].credit_expires_at.ref_type);
        expect(body[1].status).toBe(card.documentsCreatedOnMongo[1].status);
        expect(body[1].count.credits).toBe(0);
        expect(body[1].count.consumers).toBe(0);
      });
