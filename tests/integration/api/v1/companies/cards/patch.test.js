...
describe("PATCH /api/v1/companies/cards/:card_id", () => {
  ...
  describe("Authenticated user", () => {
    describe("User has a incorrect role", () => {
      test("Should return 403 status when the user is a consumer", async () => {
        ...
        const card = await orchestrator.createDocumentOnMongo(1, cardHandler, [
          {
            company_id: user.documentsCreatedOnMongo[0]._id,
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
          },
        ]);
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after create a company card", async () => {
        ...
        const response = await fetch(endpointWithCardId, {
          ...
          body: JSON.stringify({
            ...
            credit_expires_at: {
              ref_number: 1,
              ref_type: datesConstants.TYPES.MONTH,
            },
          }),
        });
        ...
        expect(body.title).toBe("Test Card Updated");
        expect(body.credits_needed).toBe(20);
        expect(body.credit_expires_at.ref_number).toBe(1);
        expect(body.credit_expires_at.ref_type).toBe(datesConstants.TYPES.MONTH);
        expect(body.status).toBe(statusConsts.RESOURCE_STATUS.AVAILABLE);
        expect(body.excluded).toBe(false);
        expect(body.company_id).toBe(company.documentsCreatedOnMongo[0]._id.toString());
      });

      test("Should return 200 status after a partial update a company card", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
          },
      ...
      test("Should return 400 when the card is not available", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
            excluded: false,
      ...
      test("Should return 400 when the card is not found", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
            excluded: false,
        ...
        const endpointWithCardId = endpoint.replace(":card_id", new mongoose.Types.ObjectId().toString());
      ...
      test("Should return 400 when the card is excluded", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
            excluded: false,
