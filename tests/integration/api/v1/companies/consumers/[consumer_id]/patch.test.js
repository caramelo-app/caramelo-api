...
describe("PATCH /api/v1/companies/consumers/:consumer_id", () => {
  describe("Anonymous user", () => {
    test("Should return 401 status when the user is not sending a token", async () => {
      const response = await fetch(`${endpoint}/123456789012345678901234`, {
        method: "PATCH",
      });
  ...
  describe("Authenticated user", () => {
    ...
    describe("User has a client role", () => {
      test("Should return 200 status after updating consumer data", async () => {
        ...
        const newPhone = dataUtils.generatePhoneNumber();

        const response = await fetch(`${endpoint}/${consumer.documentsCreatedOnMongo[0]._id}`, {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
        ...
        expect(body.message).toBe(localize("companies.consumers.update.success"));

        const consumerCheck = await userHandler.read({
          filter: {
        ...
        expect(consumerCheck).toBeDefined();
        expect(consumerCheck.name).toBe(newName);
        expect(consumerCheck.phone).toBe(newPhone);
        expect(consumerCheck.role).toBe(roleConstants.USER_ROLES.CONSUMER);
        expect(consumerCheck.status).toBe(statusConsts.RESOURCE_STATUS.AVAILABLE);
      });

      test("Should return 400 status when the user is not sending any data", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
          },
      ...
      test("Should return 404 status when the consumer is not found", async () => {
        const company = await orchestrator.createDocumentOnMongo(1, companyHandler, [
          {
            status: statusConsts.RESOURCE_STATUS.AVAILABLE,
          },
